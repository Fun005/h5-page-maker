  ...
  methods: {
    ...
    mergeProjectData(data) {
      ...
      for (const page of data.pages) {
        page.commonStyle.width =
          page.commonStyle.width || data.width || $config.canvasH5Width;
        page.commonStyle.height =
          page.commonStyle.height || data.height || $config.canvasH5Height;
    ...
    initPageData() {
      ...
      let d = JSON.parse(getStorage("H5_PAGE_MAKER_DATA")) || {};

      this.loading = false;
      this.$store.dispatch("setPrjectData", {
        ...this.mergeProjectData(
          JSON.parse(d.page_data || editorProjectConfig.getProjectConfig())
        ),
      });
    ...
    async saveFn(showMessage = true) {
      ...
      // 本地保存数据
      setStorage("H5_PAGE_MAKER_DATA", JSON.stringify(saveParams));
      // 提交数据到Boss系统
