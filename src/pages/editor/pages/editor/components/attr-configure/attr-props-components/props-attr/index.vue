<template>
  <div v-if="currentElementProps.length" class="props-attr">
    <div class="attr-title marginB10 fontBold">
      <span>组件属性</span>
      <el-link v-if="documentUrl" :href="documentUrl" target="_blank" icon="el-icon-question"></el-link>
    </div>
...
import componentsDoc from '@/constant/document'
...
export default {
  ...
  components: {
    ...
    computed: {
        ...
        documentUrl(){
          return componentsDoc[this.activeElement.elName]
        }
        /**
         * 当前选中元素需要编辑得props 列表
         */
        ...
        // 			const val = Object.keys(this.activeElement.propsValue).map(k => ({ key: k, value: this.activeElement.propsValue[k] })).filter(v => {
        // 					v = v.key.split('_')[0];
        // 					return editPropsComponentNameList.includes('attr-qk-' + v);
        // 			});
