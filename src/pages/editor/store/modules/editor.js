...
const actions = {
	...
	addPage({commit}, pageName ){
        ...
        // if(pages.findIndex(v=>v.name===pageName)>-1){
        //     alert('该状态已存在')
        //     return
        // }
	 ...
	 * @param uuid
	 */
	asyncPage({commit}, pageUuid){
		let pages = cloneDeep(state.projectData.pages)
		const activeElement = getters.activeElement(state)

		for (const page of pages) {
			if (!pageUuid || page.uuid === pageUuid) { // 同步到全部状态 或者就是这个状态
				const brotherElement = page.elements.find(element => element.brotherUuid === activeElement.brotherUuid)
				if (brotherElement) {
					Object.assign(brotherElement, activeElement, { uuid: brotherElement.uuid })
				} else {
					const data = { ...activeElement, uuid: createUUID() }
		...
		// 	const page = pages[i]
		// 	if( !pageUuid || pageUuid === page.uuid ){
		// 		for( let j=0;j< page.elements.length;j++){
		// 			let element = page.elements[j]
		// 			if (element.brotherUuid === activeElement.brotherUuid) {
		// 				page.elements[j] = {
		// 					...activeElement,
		// 					uuid:element.uuid
		// 				}
