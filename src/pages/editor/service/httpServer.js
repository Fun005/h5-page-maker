...
axios.interceptors.request.use(config => {
  config.headers.Authorization = store.getters.authorization || localStorage.getItem('token') ? 'Bearer ' + localStorage.getItem('token') : ''
  return config
}, error => {
  ...
  if (err && err.response) {
    switch (err.response.status) {
      ...
      case 401:
        ...
        // goLogin(){
        mUtils.Cookie.set('beforeLoginUrl',encodeURIComponent(router.currentRoute.fullPath), 1/24/60, window.location.host, window.location.pathname.substring(0, window.location.pathname.length-1)) // 保存用户进入的url
