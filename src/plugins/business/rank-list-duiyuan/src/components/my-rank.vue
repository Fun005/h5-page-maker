  ...
  >
    ...
    >
      <div class="my-rank">
        <span>{{myRank}}</span>
        <img
          @click="jumpToUserCenter(userInfo.uid)"
          class="my-avatar"
      ...
      <div class="my-num">
        <img
          v-if="imageSrcGiftIcon"
          class="my-icon"
          :src="imageSrcGiftIcon"
          alt=""
        >
...
export default {
  ...
  computed: {
    myRank() {
      return this.userInfo.rank + "" === "-1"
        ? "未上榜"
        : `第 ${Number(this.userInfo.rank) + 1} 名`;
    },
    infoStyle() {
      return {
