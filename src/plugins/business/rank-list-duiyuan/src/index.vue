        ...
        <ul v-if="list.length > 0" :class="cx('list-wrapper')">
            ...
            <li v-for="(item, index) in list" :class="cx('item')" :key="index">
                ...
                <div :class="cx('item-avatar')">
                    <img @click="handleAvatarClick(item.uid)" :src="item.thumb" alt=""/>
                </div>
                ...
                <div :class="cx('item-score')">
                    <img v-if="imageSrc_giftIcon" :class="cx('item-icon')" :src="imageSrc_giftIcon" alt=""/>
                    <span class="item-total"><template
                        v-if="imageSrc_giftIcon">x</template> {{ item.total || 0 }}</span>
                </div>
            </li>
        </ul>
        <my-rank
...
export default {
    ...
    computed: {
        ...
        userInfo () {
            return this.allUserInfo[this.currentPageIndex] || {
                'nick': '',    //昵称
                'portrait': '',//头像
                'total': 0,//数值
    ...
    methods: {
        ...
        handleAvatarClick (uid) {
            try {
                ...
                window.LXFlutter.postMessage(JSON.stringify({
                    message: 'userProfile',
